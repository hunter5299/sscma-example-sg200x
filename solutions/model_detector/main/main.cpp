...
int main(int argc, char** argv) {
    ...
    if (!model) {
        MA_LOGE(TAG, "ModelFactory::create failed (this solution no longer bypasses SSCMA)");
        return 1;
    }
    ...
    if (input_width != input_height) {
        MA_LOGE(TAG, "This solution only supports square input (%dx%d). Please use solutions/nonsquare_bypass_detector for non-square models.", input_width, input_height);
        return 1;
    }
