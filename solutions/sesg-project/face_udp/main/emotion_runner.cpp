...
void EmotionRunner::alignCropRgb(const uint8_t* src, int src_w, int src_h,
    ...
    // 备注（对齐裁剪）：
    // 这里与 AGR 类似：把 bbox 扩张成正方形（crop_scale_），然后双线性采样到 dst_size x dst_size。
    // 注意：此实现假设 src 是“紧密 RGB888”存储（stride = src_w*3）。
    // 如果你的输入帧存在行 padding（stride > src_w*3），需要像 AGR 那样传入 stride 并改这里的寻址。
...
void EmotionRunner::packInput(const uint8_t* rgb_hwc_u8) {
    // 备注（输入打包）：
    // - 支持 NCHW/NHWC
    // - 统一走 (px-mean)*scale 的预处理；若你的 emotion cvimodel 在 TPU 侧 fuse 了 preprocess，
    //   则应考虑像 AGR 一样做 raw passthrough（否则可能出现输入被量化成常量的问题）。
