...
void AgeGenderRaceRunner::alignCropRgb(const uint8_t* src, int src_w, int src_h,
    ...
    auto sample = [&](float sx, float sy, int c) -> uint8_t {
        ...
        const uint8_t* p00 = src + y0 * src_stride_bytes + x0 * 3;
        const uint8_t* p10 = src + y0 * src_stride_bytes + x1i * 3;
        const uint8_t* p01 = src + y1i * src_stride_bytes + x0 * 3;
        const uint8_t* p11 = src + y1i * src_stride_bytes + x1i * 3;
...
bool AgeGenderRaceRunner::parseOutputs(AgeGenderRaceResult& out) {
    ...
    if (engine_->getOutputSize() < 3) return false;

    const ma_tensor_t t_gender = engine_->getOutput(0);
    const ma_tensor_t t_age = engine_->getOutput(1);
    const ma_tensor_t t_race = engine_->getOutput(2);

    if (!t_gender.data.data || !t_age.data.data || !t_race.data.data) return false;

    const int gender_n = (int)shape_numel(engine_->getOutputShape(0));
    const int age_n = (int)shape_numel(engine_->getOutputShape(1));
    const int race_n = (int)shape_numel(engine_->getOutputShape(2));

    if (gender_n < 2 || age_n < 2 || race_n < 2) return false;

    auto argmax = [&](const ma_tensor_t& t, int n, float& score) -> int {
        int best = 0;
        ...
        score = std::exp(bestv) / std::max(1e-6f, sum);
        return best;
    };

    out.gender = argmax(t_gender, gender_n, out.gender_score);
    out.age = argmax(t_age, age_n, out.age_score);
    out.race = argmax(t_race, race_n, out.race_score);
    out.ok = true;
    return true;
}
...
bool AgeGenderRaceRunner::infer(const uint8_t* rgb888, int src_w, int src_h, int src_stride_bytes,
    ...
    alignCropRgb(rgb888, src_w, src_h, src_stride_bytes, x1, y1, x2, y2, input_rgb_.data(), dst_size);

    debug_print_patch_stats_once(input_rgb_.data(), dst_size, dst_size, src_w, src_h, src_stride_bytes,
                                 x1, y1, x2, y2, input_is_chw_, input_type_);
