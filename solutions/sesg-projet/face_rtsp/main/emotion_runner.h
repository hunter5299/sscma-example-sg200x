...
// 表情推理封装（SG2002 / CVIMODEL）
// - 输入：RGB888 原图 + bbox
// - 输出：7 类 softmax argmax
// 备注：
// - 本 runner 的职责是：对 bbox 做对齐裁剪/缩放 -> 打包成模型输入 -> 调用 EngineCVI -> softmax 取 argmax。
// - emotion 的 label 映射在 main.cpp / Python 接收端里统一维护（angry/disgust/.../neutral）。
class EmotionRunner {
public:
